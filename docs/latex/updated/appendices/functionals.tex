...
\subsection{Sparse and distributed high-dimensional targets}
\label{app:high-dimensional-functionals}

The ambient-dimension study distinguishes two ways in which the task may scale
with \(d\).  For \(d\geq3\), define sparse directions by embedding the
three-dimensional task,
\begin{equation}
    u_d^{\mathrm{sp}}=(u_3^\top,0,\ldots,0)^\top,
    \qquad
    v_d^{\mathrm{sp}}=(v_3^\top,0,\ldots,0)^\top.
    \label{eq:functional-sparse-directions}
\end{equation}
Here the added coordinates are nuisance dimensions.  To distribute signal over
all coordinates, let \(s^{(u)},s^{(v)}\in\{-1,+1\}^{\N}\) be two fixed
Rademacher sign sequences generated once from predeclared task seeds 1729 and
2718, and set
\begin{equation}
    u_d^{\mathrm{de}}
    :=\frac1d(s^{(u)}_1,\ldots,s^{(u)}_d)^\top,
    \qquad
    v_d^{\mathrm{de}}
    :=\frac1d(s^{(v)}_1,\ldots,s^{(v)}_d)^\top.
    \label{eq:functional-dense-directions}
\end{equation}
Both constructions retain unit \(\ell_1\) norm and hence a dimension-independent
label range.  The high-dimensional experiment evaluates
\(F_{\mathrm{exp}}\) under both sparse and distributed directions and
\(F_{\mathrm{vol}}\) under the distributed directions.
...
Memory and persistence study
  & VARMA, HMM, GARCH at three persistence levels
  & exponential memory with several decays and delayed recall under a contraction sweep \\
Cross-family robustness
  & VARMA, HMM, GARCH, medium persistence
...
Ambient-dimension study
  & VARMA with \(d\in\{3,10,30,100,300,500\}\)
  & sparse and distributed exponential memory; distributed Volterra \\
Resource and memory ablations
  & VARMA, medium persistence
  & Volterra and delayed recall at \(\tau\in\{10,20\}\) \\
Finite-shot study
  & VARMA, medium persistence
